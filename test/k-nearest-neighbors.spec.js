...
describe('K Nearest Neighbors', () => {
  ...
  it('Edge case search', () => {
    ...
    expect(qt.kNearestNeighbors(point, 8, 1000).length).to.equal(8);
    expect(qt.kNearestNeighbors(point, 8, 1000)[7].x).to.equal(100);
  });
  it('Lots of points', () => {
    const rect = new Rectangle(50, 50, 100, 100);
    let qt = new QuadTree(rect, 4);
    ...
    expect(qt.kNearestNeighbors(point, 10000).length).to.equal(10000);
  });
  it('Even more points', () => {
    ...
    expect(qt.kNearestNeighbors(point, 100000).length).to.equal(100000);
  });
});
